...
function Event({ user }) {
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-lg w-full h-screen flex justify-between gap-2">
      <div className="w-1/2 bg-gray-400 p-4 py-4 rounded-xl ">
      ...
      <div className="bg-gray-400 px-10 py-4 rounded-xl w-1/4">
        <h2 className="text-2xl font-bold mb-4">Participants</h2>
        {participations &&
          paidParticipants.map((participation) => (
            <div key={participation._id}
            className="flex justify-between rounded-md items-center bg-slate-50 px-4"
            >
              <h1 className="text-xl font-bold">{participation.user_id.name}</h1>
              {user && user.user.id === event.organizer_id._id && (
                <button
                  className="text-black hover:bg-gray-700 hover:text-white p-2 rounded-md text-sm font-bold bg-red-400 m-2"
          ...
          <div>
            ...
            {participations &&
              unpaidParticipants.map((participation) => (
                ...
                className="flex justify-between rounded-md items-center bg-slate-50 px-4"
                >
                  <span className="m-4 text-xl font-bold" key={participation._id}>
